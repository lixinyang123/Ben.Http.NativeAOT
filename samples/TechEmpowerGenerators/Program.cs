...
app.Get("/fortunes", async (req, res) =>
{
    ...
    System.Collections.Generic.List<(int id, string message)> model = await db.QueryAsync<(int id, string message)>("SELECT id, message FROM fortune");
...
internal struct World { public int id { get; set; } public int randomnumber { get; set; } }
